...
v0.6.1 is a security fix / bug fix - [timeseries] Ensure that timestamps are sorted when creating a TimeSeriesDataFrame - @shchur
...
- Fix int32/int64 difference between Windows and other platforms; fix mutation issue - @gradientsky always, only load previously trained models using the same version of AutoGluon that they were originally trained on. 
