...
def gather_column_features(
    ...
    for per_model_name, per_model_output in output.items():
        ...
        for feature_name in per_model_output[COLUMN_FEATURES][FEATURES]:
            ...
            if columns_share_one_feature:
                assert len("_".join(columns_share_one_feature)) == len(feature_name), f"model `{per_model_name}`'s cls feature name `{feature_name}` doesn't match `{columns_share_one_feature}`"
